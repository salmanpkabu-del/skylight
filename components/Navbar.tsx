        ...
        <div dir="ltr" className={`w-full max-w-[1340px] mx-auto px-5 sm:px-8 lg:px-12 transition-all duration-300 flex items-center justify-between ${
          ...
          <div className="hidden lg:flex items-center gap-5 text-right">
            ...
            <div
              dir="ltr"
              role="group"
              className="relative grid grid-cols-2 h-7 rounded-full p-[3px] bg-white/10 border border-white/15 backdrop-blur-sm hover:border-brand-green/40 hover:shadow-[0_0_12px_rgba(166,238,66,0.15)] active:scale-95 transition-all duration-150 cursor-pointer select-none"
              onClick={() => setLang(lang === "en" ? "ar" : "en")}
            >
              {/* Green pill slides between EN (left) and AR (right) */}
              <span
                className={`absolute inset-y-[3px] w-[calc(50%-3px)] rounded-full bg-brand-green shadow-[0_0_8px_rgba(166,238,66,0.45)] transition-all duration-[380ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                  isAr ? "left-[50%]" : "left-[3px]"
                }`}
              />
              <button type="button" aria-label="Switch to English"
                className={`relative z-10 px-2.5 py-0.5 text-[10px] font-extrabold tracking-[0.12em] rounded-full transition-colors duration-200 ${
                  !isAr ? "text-black" : "text-white/50"
                }`}
              >EN</button>
              <button type="button" aria-label="Switch to Arabic"
                className={`relative z-10 px-2.5 py-0.5 text-[10px] font-extrabold tracking-[0.12em] rounded-full transition-colors duration-200 ${
                  isAr ? "text-black" : "text-white/50"
                }`}
              >AR</button>
          ...
          <div className="flex lg:hidden items-center gap-2 z-[60]">
            ...
            <div
              dir="ltr"
              role="group"
              className="relative grid grid-cols-2 h-7 rounded-full p-[3px] bg-white/10 border border-white/15 backdrop-blur-sm hover:border-brand-green/40 active:scale-95 transition-all duration-150 cursor-pointer select-none"
              onClick={() => setLang(lang === "en" ? "ar" : "en")}
            >
              <span
                className={`absolute inset-y-[3px] w-[calc(50%-3px)] rounded-full bg-brand-green shadow-[0_0_8px_rgba(166,238,66,0.45)] transition-all duration-[380ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
