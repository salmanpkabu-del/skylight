      ...
      <section className="bg-brand-dark pt-40 pb-20 mobile-max:pt-32 mobile-max:pb-14">
        <div className="w-full max-w-[1340px] mx-auto px-5 sm:px-8 lg:px-12">
          ...
          <h1 className="text-[64px] leading-[1.0] tracking-[-3px] font-semibold text-white mb-6 md-tablet:text-[46px] mobile-max:text-[36px]">
            {b.heroTitleMain}<br />{b.heroTitleAccent}
          </h1>
          ...
          <div className="grid grid-cols-3 gap-6 md-tablet:grid-cols-2 mobile-max:grid-cols-1">
            {blogPosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group flex flex-col bg-brand-card border border-white/10 overflow-hidden hover:border-brand-green/40 transition-colors duration-300 rounded-2xl"
              >
                <div className="pkg-img-wrap relative h-56 overflow-hidden">
                  ...
                  />
                  <div className={`absolute top-4 ${isAr ? "right-4" : "left-4"} bg-brand-green text-black text-[10px] font-semibold tracking-[0.1em] uppercase px-3 py-1 rounded-full`}>
                    {post.category}
                  </div>
                </div>
                <div className="flex-1 p-6 flex flex-col gap-4">
                  ...
                  <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
                    ...
                    <ArrowIcon className="w-4 h-4 text-brand-green group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" />
                  </div>
                </div>
              </Link>
            ))}
