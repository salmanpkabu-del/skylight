"use client";

import { useEffect, useRef } from "react";
import Link from "next/link";
import { Plane, Hotel, MapPin, FileText, Moon, Car, Ship, ArrowRight, ArrowLeft } from "lucide-react";
import { useLanguage } from "@/lib/i18n/LanguageContext";

const icons = [
  <Plane className="w-7 h-7" key="plane" />,
  <Hotel className="w-7 h-7" key="hotel" />,
  <MapPin className="w-7 h-7" key="mappin" />,
  <FileText className="w-7 h-7" key="filetext" />,
  <Moon className="w-7 h-7" key="moon" />,
  <Car className="w-7 h-7" key="car" />,
  <Ship className="w-7 h-7" key="ship" />,
];

const hrefs = ["/contact", "/contact", "/packages", "/visas", "/contact", "/contact", "/contact"];

export default function Services() {
  const { t, isAr } = useLanguage();
  const ref = useRef<HTMLDivElement>(null);
  const ArrowIcon = isAr ? ArrowLeft : ArrowRight;

  useEffect(() => {
    const els = ref.current?.querySelectorAll(".reveal, .reveal-left");
    if (!els) return;
    const obs = new IntersectionObserver(
      (entries) => entries.forEach((e) => { if (e.isIntersecting) (e.target as HTMLElement).classList.add("revealed"); }),
      { threshold: 0.1, rootMargin: "0px 0px -60px 0px" }
    );
    els.forEach((el) => obs.observe(el));
    return () => obs.disconnect();
  }, []);

  return (
    <section id="services" ref={ref} className="bg-white py-20 sm:py-24 lg:py-32 overflow-hidden">
      <div className="w-full max-w-[1340px] mx-auto px-5 sm:px-8 lg:px-12">
        {/* Header */}
        <div className="reveal-left mb-16 mobile-max:mb-10">
          <p className="text-[11px] font-medium tracking-[0.2em] uppercase text-brand-dark/40 mb-3">
            {t.services.tag}
          </p>
          <h2 className="text-[56px] leading-[1.05] tracking-[-2.5px] font-semibold text-brand-dark
            md-tablet:text-[40px] mobile-max:text-[32px] mobile-max:tracking-[-1.5px]">
            {t.services.titleMain}<br />{t.services.titleAccent}
          </h2>
        </div>

        {/* Services grid */}
        <div className="grid grid-cols-4 gap-5 md-tablet:grid-cols-2 mobile-max:grid-cols-1">
          {t.services.cards.map((card, i) => (
            <Link
              key={i}
              href={hrefs[i]}
              className="reveal group flex flex-col gap-4 p-7 border border-brand-dark/10 hover:border-brand-green hover:bg-brand-dark hover:text-white transition-all duration-300"
              style={{ animationDelay: `${i * 0.07}s` }}
            >
              <div className="text-brand-dark group-hover:text-brand-green transition-colors duration-300">
                {icons[i]}
              </div>
              <div>
                <h3 className="text-base font-semibold text-brand-dark group-hover:text-white transition-colors mb-1.5">
                  {card.title}
                </h3>
                <p className="text-sm leading-6 text-brand-dark/55 group-hover:text-white/60 transition-colors">
                  {card.desc}
                </p>
              </div>
              <div className="mt-auto flex items-center gap-1.5 text-xs font-semibold tracking-[0.08em] uppercase text-brand-green opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {t.services.learnMore} <ArrowIcon className="w-3 h-3" />
              </div>
            </Link>
          ))}
          {/* CTA card */}
          <div className="reveal group flex flex-col justify-between p-7 bg-brand-dark text-white" style={{ animationDelay: "0.49s" }}>
            <div>
              <p className="text-xs font-medium tracking-[0.2em] uppercase text-brand-green/70 mb-3">{t.contact.tag}</p>
              <p className="text-lg font-semibold leading-tight text-white">{t.cta.desc.split(".")[0]}.</p>
              <p className="text-sm text-white/50 mt-2 leading-6">{t.cta.btnPlan}</p>
            </div>
            <Link
              href="/contact"
              className="btn-fill self-start mt-5 px-6 py-3 text-xs font-semibold tracking-[0.1em] uppercase"
            >
              {t.cta.btnPlan}
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
}
