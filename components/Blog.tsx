...
const posts = [
  {
    ...
    titleKey: "Top 7 Affordable Holiday Packages from UAE in 2025",
    excerptKey: "From exotic Silk Road destinations to tropical beaches, these UAE-departure packages offer incredible value without compromising on experience.",
  },
  {
    ...
    titleKey: "Complete Schengen Visa Guide for UAE Residents 2025",
    excerptKey: "Everything you need to know about applying for a Schengen visa from the UAE, including documents, timelines, appointment booking, and success tips.",
  },
  {
    ...
    titleKey: "Why Skylight is UAE's Most Trusted Travel Agency",
    excerptKey: "With 5,000+ happy travellers and a dedicated team in Sharjah, discover why UAE residents choose Skylight Travel.",
  },
];
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16 mobile-max:mb-10">
          <div className="reveal-left">
            ...
            <h2 className="text-[56px] leading-[1.05] tracking-[-2.5px] font-semibold text-brand-dark
              ...
              {t.blog.titleMain}<br />{t.blog.titleAccent}
            </h2>
          </div>
          <Link
            href="/blog"
          ...
          {posts.map((post, i) => (
            ...
            >
              ...
              <div className="flex-1 p-6 flex flex-col gap-4">
                ...
                <p className="text-sm leading-6 text-brand-dark/55">{post.excerptKey}</p>
                <div className="mt-auto flex items-center gap-2 text-xs font-semibold tracking-[0.08em] uppercase text-brand-dark/40 group-hover:text-brand-dark transition-colors">
                  {t.blog.readArticle} <ArrowIcon className="w-3 h-3 group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" />
                </div>
              </div>
            </Link>
          ))}
