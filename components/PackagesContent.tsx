...
import { Clock, Star, Users, ArrowRight, ArrowLeft, Sparkles, CheckCircle2, MessageCircle, HelpCircle } from "lucide-react";
...
export default function PackagesContent() {
  ...
  const CardArrow = isAr ? ArrowLeft : ArrowRight;
          ...
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs text-white/80 font-medium">
              <span className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md">
                <CheckCircle2 className="w-4 h-4 text-brand-green" /> {p.badgeFrom}
              </span>
              <span className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md">
                <CheckCircle2 className="w-4 h-4 text-brand-green" /> {p.badgeFlightHotel}
              </span>
              <span className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md">
                <MessageCircle className="w-4 h-4 text-[#25D366]" /> {p.badgeWhatsApp}
              </span>
            </div>
          </div>
        </div>
            ...
            {packages.map((pkg) => (
              ...
              >
                ...
                <div className="relative h-60 w-full overflow-hidden bg-[#012119]">
                  ...
                  {/* Badge */}
                  <div className={`absolute top-4 ${isAr ? "right-4" : "left-4"} bg-brand-green text-black text-[10px] font-bold tracking-[0.15em] uppercase px-3 py-1 rounded-full shadow-[0_0_15px_rgba(166,238,66,0.3)]`}>
                    {pkg.badge}
                  </div>
                ...
                <div className="flex-1 p-6 flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-white/70 pt-1">
                    ...
                    <span className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-2.5 py-1 rounded-lg">
                      <Users className="w-3.5 h-3.5 text-brand-green" /> {pkg.groupSize} {t.common.pax}
                    </span>
                    <span className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-2.5 py-1 rounded-lg">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-white tracking-tight">
                        ...
                        <span className="text-xs font-normal text-white/50">{t.common.perPerson}</span>
                      </p>
                    </div>
                    <div className="w-10 h-10 rounded-full bg-white/5 group-hover:bg-brand-green group-hover:text-black flex items-center justify-center text-brand-green transition-all duration-300">
                      <CardArrow className="w-5 h-5 group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 transition-transform" />
                    </div>
                  </div>
                </div>
              </Link>
      ...
      <section className="bg-[#012119] py-20 lg:py-24 border-t border-white/10">
        <div className="w-full max-w-[1340px] mx-auto px-5 sm:px-8 lg:px-12">
          <div className="max-w-3xl mx-auto text-center mb-14">
            <p className="text-[11px] font-semibold tracking-[0.2em] uppercase text-brand-green mb-2">{p.faqTag}</p>
