...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} className="bg-brand-section overflow-hidden py-24 lg:py-32 border-t border-white/10">
      <div className="w-full max-w-[1340px] mx-auto px-5 sm:px-8 lg:px-12 flex flex-col gap-16 lg:gap-20">
        ...
        <div className="reveal flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          <h2 className="text-[42px] sm:text-[56px] lg:text-[72px] leading-[1.05] tracking-[-2px] sm:tracking-[-3px] font-semibold text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* Main Story Card */}
          <div className="reveal lg:col-span-8 bg-brand-card/90 rounded-[32px] p-10 sm:p-14 flex flex-col justify-between border border-white/10 shadow-xl">
            <div className="space-y-6 text-[18px] sm:text-[22px] leading-[1.6] text-white/90 font-medium">
              <p>
                Founded in 2014 in Al Sajaa, Sharjah, Skylight Travel was built on a single belief: <span className="text-brand-green font-semibold">every family deserves to experience the world without the stress of planning it alone.</span>
              </p>
              <p className="text-[16px] sm:text-[18px] leading-[1.8] text-white/60 font-light">
            ...
            <div className="mt-12 pt-8 border-t border-white/10">
              <Link
                href="/about"
                className="inline-flex items-center gap-3 text-[12px] font-bold tracking-[0.15em] uppercase text-white hover:text-brand-green transition-colors w-fit group"
              >
                Read Our Full Story
                <span className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center group-hover:border-brand-green transition-colors">
          ...
          <div className="reveal lg:col-span-4 bg-[#012119] rounded-[32px] p-10 sm:p-14 flex flex-col justify-center items-center text-center relative overflow-hidden group border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 pt-8">
          {pillars.map(({ icon: Icon, label, desc }, i) => (
            <div key={label} className="reveal flex flex-col gap-4 group" style={{ transitionDelay: `${i * 100}ms` }}>
              <div className="w-12 h-12 rounded-full bg-brand-green/10 flex items-center justify-center text-brand-green group-hover:bg-brand-green group-hover:text-black transition-colors duration-300">
                <Icon className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-[18px] font-semibold text-white mb-2 group-hover:text-brand-green transition-colors">{label}</h3>
                <p className="text-[15px] leading-[1.7] text-white/60 font-light">{desc}</p>
              </div>
            </div>
          ))}
        </div>
